...
class BarShell extends LitElement implements MicroAppElement {
  ...
  basepath='/apps/bar-manager'; route='/'; env;
  async connectedCallback(){ super.connectedCallback(); await ensure('bar'); this.env?.bus?.addEventListener(LOCALE_CHANGED, ()=>this.requestUpdate()); }
  render(){ return html`<h2>${t('bar','title')}</h2><p>${t('bar','intro')}</p>`; }
}
