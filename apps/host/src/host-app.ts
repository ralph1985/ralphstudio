...
import { ensure, t, getLocale, setLocale } from './i18n';
import '@ralphstudio/ui/src/components/Button.ts';

export class HostApp extends LitElement {
  static styles = css`:host{display:block;max-width:1200px;margin:0 auto;padding:16px}`;
  private bus = new EventTarget();

  async connectedCallback(){
    super.connectedCallback();
    await ensure('ui'); await ensure('host');
    document.documentElement.lang = getLocale();
  }

  private changeLocale(l:'es'|'en'){ setLocale(l, this.bus); this.requestUpdate(); }

  private async mount(slug:string){
    try{
      await loadRemote(slug);
      const tag = slug==='buybuddies'?'bb-shell':slug==='bar-manager'?'bar-shell':'dlv-shell';
    ...
    }catch(_e){ this.renderRoot.querySelector('#outlet')!.textContent = 'Error cargando app.'; }
  }
    ...
    return html`
      <header style="display:flex;gap:8px;justify-content:space-between;align-items:center">
        ...
        <div role="group" aria-label="Idioma">
          <button @click=${()=>this.changeLocale('es')} aria-pressed=${getLocale()==='es'}>ES</button>
          <button @click=${()=>this.changeLocale('en')} aria-pressed=${getLocale()==='en'}>EN</button>
        </div>
      </header>
