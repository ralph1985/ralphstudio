...
export async function loadRemote(slug: string) {
  ...
  const entry = v ? `/remotes/${slug}/${v}/remote.js` : mf[slug]?.channels?.[channel]?.entry;

  if (!entry) throw new Error(`entry_not_found:${slug}:${channel}`);
  const mod = await import(entry);

  if (typeof (mod as RemoteModule).register !== 'function') throw new Error('register_missing');
  (mod as RemoteModule).register();
}
