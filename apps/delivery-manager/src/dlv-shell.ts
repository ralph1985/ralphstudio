...
class DlvShell extends LitElement implements MicroAppElement {
  ...
  async connectedCallback(){ super.connectedCallback(); await ensure('delivery'); this.env?.bus?.addEventListener(LOCALE_CHANGED, ()=>this.requestUpdate()); }
  render(){ return html`<h2>${t('delivery','title')}</h2><p>${t('delivery','intro')}</p>`; }
}
